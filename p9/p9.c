...
int p9_write_full(struct p9conn *p9, const uint8_t *buf, int len) {
    if (atomic_load(&p9->connection_dead)) return -1;

    if (p9->ssl) {
        int r = tls_write_full(p9->ssl, buf, len);
        if (r < 0) {
            wlr_log(WLR_ERROR, "9P write: TLS connection lost");
            atomic_store(&p9->connection_dead, 1);
            return -1;
        }
        return r;
    }
...
int p9_read_full(struct p9conn *p9, uint8_t *buf, int n) {
    ...
    if (p9->ssl) {
        ...
        if (r < 0) {
            wlr_log(WLR_ERROR, "9P read: TLS connection lost");
            atomic_store(&p9->connection_dead, 1);
            return -1;
        }
        return r;
 ...
 *
 * Thread-safety note: p9_walk_open is the one fid-allocating helper that
 * runs on connections shared across threads.  s->p9_snarf is touched
 * concurrently by the event-loop wayland_to_snarf path (which calls
 * p9_write_file) and the detached snarf_read_thread (which calls
 * p9_read_file).  A non-atomic post-increment on p9->next_fid lets two
 * callers compute the same fid value, after which the second walk
 * either fails ("fid in use") or aliases the first — either way the
 * stream desyncs.  We grab p9->lock just for the increment.  Holding
 * across p9_walk and p9_open isn't necessary (and would serialize
 * unrelated operations on the connection); each of those takes the
 * lock itself for its own RPC.
 *
 * Per-window connections (p9_draw / p9_relookup / p9_mouse / p9_kbd /
 * p9_wctl) are owned by a single thread each, so the lock contention
 * here is essentially zero on those.
 */
...
int p9_connect(struct p9conn *p9, const char *host, int port,
     ...
     * Start probing after 5s idle, probe every 5s, give up after 3 failures.
     * Without this, a dead host can leave reads blocked for minutes. */
    setsockopt(p9->fd, SOL_SOCKET, SO_KEEPALIVE, &one, sizeof(one));
    int idle = 5, interval = 5, count = 3;
    setsockopt(p9->fd, IPPROTO_TCP, TCP_KEEPIDLE, &idle, sizeof(idle));
    setsockopt(p9->fd, IPPROTO_TCP, TCP_KEEPINTVL, &interval, sizeof(interval));
    setsockopt(p9->fd, IPPROTO_TCP, TCP_KEEPCNT, &count, sizeof(count));
