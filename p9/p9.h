 ...
 * Error Handling:
 *
 *   Connection errors (socket failures, TLS errors) set the connection_dead
 *   flag and return -1. Callers must check for errors and initiate shutdown.
 *   Once connection_dead is set, all subsequent p9_read_full / p9_write_full
 *   calls short-circuit immediately without touching the (broken) socket.
 *
 ...
 *     - connection_dead: TCP/TLS connection lost
 *
 * Usage:
 *
 *   struct p9conn p9;
...
struct p9conn {
    ...
    atomic_int connection_dead;   /* TCP/TLS connection lost (socket error or EOF) */
};
 ...
 * On error, sets p9->connection_dead and returns -1.
 *
 * p9:  connection
 * buf: data to write
 * len: number of bytes
