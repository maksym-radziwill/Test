...
static void ensure_initialized(void) {
    pthread_once(&pool_once, do_pool_init);
}
...
void parallel_for(int count, parallel_fn fn, void *ctx) {
    ...
    /* Wait for any in-progress parallel_for from another send thread.
     * pthread_cond_wait releases pool.lock, which lets a second caller
     * overwrite fn/ctx/count while the first is still waiting — that's
     * the bug this busy flag prevents. */
    while (pool.busy)
    ...
    pool.busy = 0;
    pthread_cond_broadcast(&pool.done_cond);  /* wake next waiting caller */
    pthread_mutex_unlock(&pool.lock);
}
