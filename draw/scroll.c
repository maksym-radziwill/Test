...
static void detect_region_scroll(void *ctx, int reg_idx) {
    ...
    struct rio_window *rw = sc->rw;
    uint32_t *send_buf = sc->send_buf;
    uint32_t *prev_buf = rw->prev_framebuf;
    int width = rw->width;
    
    rw->scroll_regions[reg_idx].detected = 0;
    rw->scroll_regions[reg_idx].dx = 0;
    rw->scroll_regions[reg_idx].dy = 0;
    
    int rx1 = rw->scroll_regions[reg_idx].x1;
    int ry1 = rw->scroll_regions[reg_idx].y1;
    int rx2 = rw->scroll_regions[reg_idx].x2;
    int ry2 = rw->scroll_regions[reg_idx].y2;
    ...
    for (int ty = ty1; ty < ty2; ty++) {
        for (int tx = tx1; tx < tx2; tx++) {
            ...
            if (!in_exposed && src_x1 >= 0 && src_y1 >= 0 &&
                src_x1 + w <= rw->width && src_y1 + h <= rw->height) {
                int identical = 1;
    ...
    rw->scroll_regions[reg_idx].detected = 1;
    rw->scroll_regions[reg_idx].dx = dx;
    rw->scroll_regions[reg_idx].dy = dy;
...
void detect_scroll(struct rio_window *rw, uint32_t *send_buf) {
    if (!send_buf || !rw->prev_framebuf) return;
    
    double t_start = get_time_us();
    memset(&timing, 0, sizeof(timing));
    
    int margin = TILE_SIZE;
    int cols = rw->width / 256 > 0 ? rw->width / 256 : 1;
    int rows = rw->height / 256 > 0 ? rw->height / 256 : 1;
    int cell_w = ((rw->width - 2 * margin) / cols / TILE_SIZE) * TILE_SIZE;
    int cell_h = ((rw->height - 2 * margin) / rows / TILE_SIZE) * TILE_SIZE;
    
    if (cell_w < TILE_SIZE) cell_w = TILE_SIZE;
    if (cell_h < TILE_SIZE) cell_h = TILE_SIZE;
    
    rw->scroll_regions_x = cols;
    rw->scroll_regions_y = rows;
    rw->num_scroll_regions = 0;
    
    int max_x = (rw->width / TILE_SIZE) * TILE_SIZE;
    int max_y = (rw->height / TILE_SIZE) * TILE_SIZE;
    
    /*
     * Cap total regions at MAX_SCROLL_REGIONS.
     *
     * For modest displays (1080p, 4K) cols*rows is well under 128, but
     * 5K+ displays can produce >256 regions.  Without a cap the
     * `rw->scroll_regions[idx] = ...` writes below would overrun the
     * fixed-size array and corrupt adjacent rio_window fields.
     *
     * If we'd exceed the cap, recompute coarser cells so the same
     * screen area is covered by fewer larger regions.  Larger cells
     * still have plenty of texture for FFT correlation (the algorithm
     * only needs >= 16 px per side; cells are 256+ even after capping).
     */
    if (cols * rows > MAX_SCROLL_REGIONS) {
        wlr_log(WLR_INFO, "scroll: %dx%d grid (%d regions) exceeds cap %d, "
                "coarsening grid", cols, rows, cols * rows, MAX_SCROLL_REGIONS);
        /* Scale both dimensions down proportionally.  sqrt of the ratio
         * preserves the aspect of the original grid. */
        double scale = (double)MAX_SCROLL_REGIONS / (cols * rows);
        double k = sqrtf(scale);
        cols = (int)(cols * k);
        rows = (int)(rows * k);
        if (cols < 1) cols = 1;
        if (rows < 1) rows = 1;
        /* Final clamp in case the rounding bumped us back over. */
        while (cols * rows > MAX_SCROLL_REGIONS) {
            if (cols >= rows) cols--;
            else rows--;
        }
        cell_w = ((rw->width - 2 * margin) / cols / TILE_SIZE) * TILE_SIZE;
        cell_h = ((rw->height - 2 * margin) / rows / TILE_SIZE) * TILE_SIZE;
        if (cell_w < TILE_SIZE) cell_w = TILE_SIZE;
        if (cell_h < TILE_SIZE) cell_h = TILE_SIZE;
        rw->scroll_regions_x = cols;
        rw->scroll_regions_y = rows;
    ...
    for (int ry = 0; ry < rows; ry++) {
        for (int rx = 0; rx < cols; rx++) {
            ...
            int x2 = (rx == cols - 1)
                ? ((rw->width - margin) / TILE_SIZE) * TILE_SIZE
                : ((x1 + cell_w + TILE_SIZE - 1) / TILE_SIZE) * TILE_SIZE;
            int y2 = (ry == rows - 1)
                ? ((rw->height - margin) / TILE_SIZE) * TILE_SIZE
            ...
            /* Defensive bounds check.  The cap above should make this
             * unreachable, but guard anyway: silently dropping a region
             * is better than corrupting memory. */
            if (rw->num_scroll_regions >= MAX_SCROLL_REGIONS) {
                wlr_log(WLR_ERROR, "scroll: region cap reached unexpectedly, dropping rest");
                ry = rows; break;
            }
            
            int idx = rw->num_scroll_regions++;
            rw->scroll_regions[idx].x1 = x1;
            rw->scroll_regions[idx].y1 = y1;
            rw->scroll_regions[idx].x2 = x2;
            rw->scroll_regions[idx].y2 = y2;
            rw->scroll_regions[idx].detected = 0;
            rw->scroll_regions[idx].dx = 0;
            rw->scroll_regions[idx].dy = 0;
        }
    }
    
    if (rw->num_scroll_regions > 0) {
        struct scroll_ctx ctx = { .rw = rw, .send_buf = send_buf };
        parallel_for(rw->num_scroll_regions, detect_region_scroll, &ctx);
    }
    
    int detected_count = 0;
    for (int i = 0; i < rw->num_scroll_regions; i++) {
        if (rw->scroll_regions[i].detected) {
            detected_count++;
    ...
    timing.regions_processed = rw->num_scroll_regions;
    
    if (detected_count > 0) {
        wlr_log(WLR_INFO, "Scroll detected in %d/%d regions (%.1fus)",
                detected_count, rw->num_scroll_regions, timing.total_us);
    }
}

int apply_scroll_to_prevbuf(struct rio_window *rw) {
    int scrolled_count = 0;
    
    for (int i = 0; i < rw->num_scroll_regions; i++) {
        if (!rw->scroll_regions[i].detected) continue;
        
        int rx1 = rw->scroll_regions[i].x1;
        int ry1 = rw->scroll_regions[i].y1;
        int rx2 = rw->scroll_regions[i].x2;
        int ry2 = rw->scroll_regions[i].y2;
        int dx = rw->scroll_regions[i].dx;
        int dy = rw->scroll_regions[i].dy;
        ...
        if (dy < 0) {
            for (int y = r.dst_y1; y < r.dst_y2; y++) {
                memmove(&rw->prev_framebuf[y * rw->width + r.dst_x1],
                        &rw->prev_framebuf[(y + abs_dy) * rw->width + r.src_x1],
            ...
            for (int y = r.dst_y1; y < r.dst_y2; y++) {
                memmove(&rw->prev_framebuf[y * rw->width + r.dst_x1],
                        &rw->prev_framebuf[y * rw->width + r.src_x1],
                        copy_w * sizeof(uint32_t));
            }
        }
        ...
        if (r.exp_y2 > r.exp_y1) {
            for (int y = r.exp_y1; y < r.exp_y2; y++) {
                for (int x = rx1; x < rx2; x++) {
                    rw->prev_framebuf[y * rw->width + x] = 0xDEADBEEF;
                }
            }
        }
...
int write_scroll_commands(struct rio_window *rw, uint8_t *batch, size_t max_size) {
