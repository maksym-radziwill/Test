...
void input_queue_init(struct input_queue *q) {
    ...
    /*
     * Both ends non-blocking.  The read end is drained (drain-on-wake
     * loop in handle_input_events).  The write end matters because
     * input_queue_push writes to it WHILE HOLDING q->lock; if the pipe
     * filled (~64 KB on Linux) and the write blocked, other input
     * threads waiting on q->lock would deadlock until the main loop
     * caught up.  EAGAIN on a coalesced wakeup is harmless: the event
     * is already on the ring buffer and the next successful write or
     * the existing pipe contents will wake the consumer, which drains
     * the entire ring buffer in one pass.
     */
    fcntl(q->pipe_fd[0], F_SETFL, O_NONBLOCK);
    fcntl(q->pipe_fd[1], F_SETFL, O_NONBLOCK);
}
...
void *mouse_thread_func(void *arg) {
    ...
    while (s->running) {
        ...
        if (buf[0] == 'm') {
            ...
            atomic_store(&rw->window_changed, 1);
    ...
    /* Connection lost or error.  If this is the primary window AND no
     * other rio windows are alive, the compositor has nothing left to
     * display, so terminate cleanly.  In -W mode with secondaries still
     * alive (e.g. Firefox/Chromium in their own rio windows), we just
     * exit this thread; the main loop continues serving the others.
     * For secondaries we always just exit the thread. */
    if (s->running && rw == s->primary &&
        !(s->multi_window && has_other_live_windows(s, rw))) {
        wlr_log(WLR_ERROR, "Mouse thread: primary 9P connection lost, initiating shutdown");
        s->running = 0;
        wl_display_terminate(s->display);
    } else if (rw) {
        if (rw == s->primary)
            wlr_log(WLR_INFO, "Mouse thread: primary connection lost; "
                    "secondaries still alive, exiting thread only");
        else
            wlr_log(WLR_INFO, "Mouse thread[win%d]: secondary connection lost, exiting", rw->id);
        /*
         * Coordinate with the other worker threads on this rw (kbd, send).
         * When all three have exited, the last one out posts
         * INPUT_RIO_WINDOW_DIED so the event loop can clean up:
         *   - primary: promote a surviving secondary
         *   - secondary: migrate any toplevels off and destroy
         *
         * The being_destroyed check skips the post when multiwin_destroy_window
         * is already tearing this rw down (toplevel-close path).  Otherwise
         * the event loop would race against the in-progress destroy and
         * touch a freed rw.
         */
        if (s->running && !atomic_load(&rw->being_destroyed) &&
            atomic_fetch_add(&rw->threads_exited, 1) + 1 >= 3) {
            struct input_event ev = { .type = INPUT_RIO_WINDOW_DIED, .window = rw };
            input_queue_push(&s->input_queue, &ev);
        }
    }
...
void *kbd_thread_func(void *arg) {
    ...
    /* Connection lost or error — see mouse_thread_func for rationale. */
    if (s->running && rw == s->primary &&
        !(s->multi_window && has_other_live_windows(s, rw))) {
        wlr_log(WLR_ERROR, "Kbd thread: primary 9P connection lost, initiating shutdown");
        s->running = 0;
        wl_display_terminate(s->display);
    } else if (rw) {
        if (rw == s->primary)
            wlr_log(WLR_INFO, "Kbd thread: primary connection lost; "
                    "secondaries still alive, exiting thread only");
        else
            wlr_log(WLR_INFO, "Kbd thread[win%d]: secondary connection lost, exiting", rw->id);
        /* Last thread out posts INPUT_RIO_WINDOW_DIED — see mouse_thread_func. */
        if (s->running && !atomic_load(&rw->being_destroyed) &&
            atomic_fetch_add(&rw->threads_exited, 1) + 1 >= 3) {
            struct input_event ev = { .type = INPUT_RIO_WINDOW_DIED, .window = rw };
            input_queue_push(&s->input_queue, &ev);
