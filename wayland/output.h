...
/*
 * Pipe-readable handler for the presentation feedback channel.
 *
 * Registered by main.c (and multiwin.c for secondary windows) on
 * rw->present_pipe[0].  Fires when the send thread has finished
 * delivering a frame to rio and posted a wakeup byte.
 *
 * Reads (and discards) all queued bytes from the pipe — multiple
 * completions coalesce into one wlr_scene_output_send_frame_done
 * call.  If frame_done_pending is clear, the most recent output
 * frame already fired frame_done synchronously (idle path), and
 * this is a no-op.
 *
 * Returns 0 (event source remains registered until removed
