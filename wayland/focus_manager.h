...
struct popup_data {
    ...
    struct rio_window *window;      /* Which rio window owns this popup */
...
/*
 * Clean up all focus state associated with a dying rio_window.
 *
 * When a rio window dies (primary deleted, secondary toplevel closed),
 * its kbd/mouse threads stop generating events. Any modifiers or
 * buttons that were held at death are stuck in global state forever:
 *
 *   - modifier_state bits set by this window's kbd thread → stuck
 *     modifiers on all surviving windows (e.g. Ctrl+scroll = zoom)
 *   - seat button_count incremented by this window's mouse events →
 *     BUTTONS_HELD() permanently true, all focus changes deferred
 *   - deferred_pointer_target may reference surfaces on this window
 *
 * This function uses per-window tracking (rw->held_modifiers,
 * rw->last_buttons) to undo exactly this window's contributions
 * to global state, then rechecks focus for surviving windows.
 *
 * Must be called on the event loop thread, before thread teardown.
 *
 * fm: focus manager
 * rw: rio window that is dying
 */
