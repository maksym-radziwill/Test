 ...
 *
 * In multi-window mode, if the key arrives from a rio window that
 * doesn't currently own keyboard focus, focus is switched to a
 * mapped toplevel on that window before dispatching the key.
 *
 ...
 *
 * s:       server instance
 * rw:      rio window the key event came from (NULL for single-window)
